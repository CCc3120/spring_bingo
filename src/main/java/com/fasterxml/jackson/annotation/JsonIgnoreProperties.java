...
public @interface JsonIgnoreProperties
{
    ...
    public boolean isShow() default false;

    public Class<?> clazz() default BaseModel.class;
            ...
            java.io.Serializable
    {
        ...
        public static Value from(JsonIgnoreProperties src) {
            ...
            if (src.isShow()) {
                ...
                Set<String> inner = _asSet(src.value());

                set.removeAll(inner);

                // 框架内需要的，不必序列化
                set.add("modelFromClass");

                // 内置的特殊处理，判断是否要忽略
                if (inner.contains("handler")) {
                    set.add("handler");
                }
